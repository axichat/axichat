...
- Subject tokens (`[s:XXXX]`) are injected automatically to improve reply correlation. Builders can disable tokens per send by passing `useSubjectToken: false` to `EmailService.fanOutSend`.
