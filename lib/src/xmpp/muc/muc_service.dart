...
const _mucJoinManagerJoinTimeoutLog =
    'moxxmpp joinRoom future lagged after room readiness.';
const _mucJoinManagerTimeoutBeforeReadyLog =
    'moxxmpp joinRoom still pending before room readiness.';
const _mucJoinManagerErrorAfterReadyLog =
    'moxxmpp joinRoom returned an error after room readiness.';
const _mucIdleLeaveFailedLog = 'Failed to leave idle room.';
...
const _mucPendingInviteesStorageKeyPrefix = 'muc_pending_invitees:';
...
final class _MucRoomSession {
  ...
  bool waitingForLateSelfPresence = false;
  bool suppressSnapshotPersistence = false;
  ...
  void cancelIdleLeaveTimer() {
    idleLeaveTimer?.cancel();
    idleLeaveTimer = null;
  }
...
final class _PersistedRoomMember {
  ...
  _PersistedRoomMember _mergeFallback(_PersistedRoomMember fallback) {
    final resolvedNick = nick.trim().isNotEmpty ? nick : fallback.nick;
    final resolvedOccupantId = occupantId?.trim();
    final fallbackOccupantId = fallback.occupantId?.trim();
    final resolvedRealJid = realJid?.trim();
    final fallbackRealJid = fallback.realJid?.trim();
    return _PersistedRoomMember(
      nick: resolvedNick,
      occupantId: resolvedOccupantId?.isNotEmpty == true
          ? resolvedOccupantId
          : fallbackOccupantId?.isNotEmpty == true
          ? fallbackOccupantId
          : null,
      realJid: resolvedRealJid?.isNotEmpty == true
          ? resolvedRealJid
          : fallbackRealJid?.isNotEmpty == true
          ? fallbackRealJid
          : null,
      affiliation: affiliation.isNone ? fallback.affiliation : affiliation,
      role: role.isNone ? fallback.role : role,
    );
  }
...
mixin MucService on XmppBase, BaseStreamService, AvatarService, MessageService {
  ...
  static const Duration _mucIdleLeaveDelay = Duration(minutes: 5);
  ...
  void _clearRoomSelfPresenceReadiness(String roomJid) {
    _roomSessionForKey(_roomKey(roomJid))?.clearSelfPresenceReadiness();
  }

  void _clearAllRoomSelfPresenceReadiness() {
    _selfPresenceGeneration++;
    for (final session in _roomSessions.values) {
      session.clearSelfPresenceReadiness();
    }
  }
  ...
  bool _roomWaitingForLateSelfPresence(String roomJid) =>
      _roomSessionForKey(_roomKey(roomJid))?.waitingForLateSelfPresence == true;

  bool _roomHasCurrentSelfPresence(String roomJid) =>
      _roomSessionForKey(_roomKey(roomJid))?.selfPresenceGeneration ==
      _selfPresenceGeneration;

  bool _roomPresenceRetained(String roomJid) =>
      (_roomSessionForKey(_roomKey(roomJid))?.presenceRetainCount ?? 0) > 0;

  bool _roomIdleLeaveInFlight(String roomJid) =>
      _roomSessionForKey(_roomKey(roomJid))?.idleLeaveFuture != null;

  Future<void> _awaitIdleLeaveTaskIfActive(String roomJid) async {
    final key = _roomKey(roomJid);
    final idleLeaveFuture = _roomSessionForKey(key)?.idleLeaveFuture;
    if (idleLeaveFuture == null) return;
    await idleLeaveFuture;
  }

  String? _normalizedMucNick(String? nick) {
  ...
  String _retainedRoomRejoinNickname(String roomJid) {
    final key = _roomKey(roomJid);
    return _normalizedMucNick(_roomNicknameForKey(key)) ??
        _normalizedMucNick(_roomStates[key]?.selfNick) ??
        _nickForRoom(null);
  }

  bool _isTerminalSelfUnavailable(MucSelfPresenceEvent event) =>
      event.isError ||
      event.isRoomDestroyed ||
      event.hasStatus(MucStatusCode.kicked) ||
      event.hasStatus(MucStatusCode.banned) ||
      event.hasStatus(MucStatusCode.roomShutdown) ||
      event.hasStatus(MucStatusCode.removedByAffiliationChange) ||
      event.hasStatus(MucStatusCode.removedByMembersOnlyChange);

  bool _isRetainedStaleSelfUnavailable({
    required String roomJid,
    required MucSelfPresenceEvent event,
  }) {
    if (!event.shouldLeaveRoom) return false;
    if (_isTerminalSelfUnavailable(event)) return false;
    final key = _roomKey(roomJid);
    if (_roomExplicitlyLeft(key)) return false;
    return _roomPresenceRetained(key);
  }

  Future<void> _rejoinRetainedRoom(String roomJid) async {
    final key = _roomKey(roomJid);
    final session = _roomSessionForKey(key);
    if (session == null || session.presenceRetainCount <= 0) return;
    if (_roomExplicitlyLeft(key)) return;
    if (_roomStates[key]?.hasTerminalExit == true) return;
    session.suppressSnapshotPersistence = true;
    try {
      await ensureJoined(
        roomJid: key,
        nickname: _retainedRoomRejoinNickname(key),
        maxHistoryStanzas: 0,
        password: _passwordForRoom(key),
        allowRejoin: true,
        forceRejoin: true,
      );
    } finally {
      if (identical(_roomSessionForKey(key), session)) {
        session.suppressSnapshotPersistence = false;
      }
    }
  }

  bool _hasCurrentMucPresenceForSend({
    required String roomJid,
    String? expectedNick,
  }) {
    ...
    if (_roomIdleLeaveInFlight(normalizedRoom)) return false;
    if (!_roomHasCurrentSelfPresence(normalizedRoom)) return false;
    final roomState = roomStateFor(normalizedRoom);
    if (roomState == null) return false;
    if (_instantRoomPending(normalizedRoom)) return false;
    if (roomState.roomCreated) return false;
    if (_roomNeedsJoin(normalizedRoom)) return false;
    if (!roomState.isReadyForMessaging) return false;
    final resolvedExpectedNick =
        _normalizedMucNick(expectedNick) ??
        _normalizedMucNick(_roomNicknameForKey(normalizedRoom));
    if (resolvedExpectedNick == null) return true;
    return _normalizedMucNick(roomState.selfNick) == resolvedExpectedNick;
  }
  ...
  Future<void> _reset() async {
    for (final session in _roomSessions.values) {
      session.cancelIdleLeaveTimer();
      session.idleLeaveFuture = null;
      session.presenceRetainCount = 0;
      session.clearSelfPresenceReadiness();
    }
  ...
  MucActorIdentity _outboundMessageIdentityForChat({
    ...
    if (chatType != ChatType.groupChat) {
      return super._outboundMessageIdentityForChat(
        chatJid: chatJid,
        accountJid: accountJid,
        chatType: chatType,
      );
    }
    return _outboundMucActorIdentity(roomJid: chatJid, accountJid: accountJid);
  }
  ...
  Future<bool> _isInboundPinClearAllAuthorized(mox.MessageEvent event) async {
    ...
    if (_mucPinClearAllSenderCanManagePins(
      senderJid: senderJid,
      roomState: roomState,
    )) {
      return true;
    ...
    return await _queriedMucPinClearAllSenderCanManagePins(
      roomJid: roomJid,
      senderJid: senderJid,
    );
  }
  ...
  Set<String> _selfPinnerJidsForChat({
    ...
    final pinnerJids = <String>{
      ...super._selfPinnerJidsForChat(
        chatJid: chatJid,
        chatType: chatType,
    ...
    void addPinner(String? pinnerJid) {
      final trimmed = pinnerJid?.trim();
      if (trimmed == null || trimmed.isEmpty) {
        return;
      }
      pinnerJids.add(trimmed);
    }

    void addBarePinner(String? pinnerJid) {
      final trimmed = pinnerJid?.trim();
      if (trimmed == null || trimmed.isEmpty) {
        return;
      }
      pinnerJids.add(bareAddress(trimmed) ?? trimmed);
    }

    final roomState = roomStateFor(chatJid);
    addBarePinner(myJid);
    addBarePinner(roomState?.resolvedSelfJid(fallbackJid: myJid));
    addPinner(roomState?.myOccupantJid);
    final selfNick = roomState?.selfNick?.trim();
    if (selfNick != null && selfNick.isNotEmpty) {
      addPinner('${bareAddress(chatJid) ?? chatJid}/$selfNick');
    }
    return pinnerJids;
  }
  ...
  _PendingInboundPinMutation _canonicalizeInboundPinMutationForTarget({
    ...
    if (chatType != ChatType.groupChat) {
      return super._canonicalizeInboundPinMutationForTarget(
        chatJid: chatJid,
        chatType: chatType,
        message: message,
        mutation: mutation,
    ...
    final eventSenderJid = mutation.event.from.toString();
    final eventSenderMatchesSelfMessage =
        message.isFromAccount(myJid) &&
        (message.senderJid.trim() == eventSenderJid ||
            message.occupantID?.trim() == eventSenderJid);
    if (!_isSelfPinMutation(mutation.event) &&
        !_isSelfArchivedGroupChatMessageEvent(mutation.event) &&
        !eventSenderMatchesSelfMessage) {
      return mutation;
    }
    return mutation.withPinner(
  ...
  bool _isInboundPinSenderAuthorized({
    ...
    if (chatType != ChatType.groupChat) {
      return super._isInboundPinSenderAuthorized(
        chatJid: chatJid,
        chatType: chatType,
        mutation: mutation,
      );
    ...
    if (roomState != null) {
      ...
      if (occupant != null) {
        final canOwnPin =
            occupant.role.isParticipant ||
            occupant.role.canManagePins ||
            occupant.affiliation.isMember ||
            occupant.affiliation.canManagePins;
        if (canOwnPin || _isArchivedOrOfflineMessage(mutation.event)) {
          return true;
        }
        return false;
      }
    ...
    return _isArchivedOrOfflineMessage(mutation.event);
  }
  ...
  bool _mucPinClearAllSenderCanManagePins({
    required String senderJid,
    required RoomState? roomState,
  }) {
  ...
  Future<bool> _queriedMucPinClearAllSenderCanManagePins({
    required String roomJid,
    required String senderJid,
  }) async {
  ...
  Occupant? _exactPresentMucOccupantForSender({
    ...
    final roomState = roomStateFor(bareAddress(chatJid) ?? chatJid);
    if (roomState == null) {
      return null;
    }
    final occupantId = roomState.canonicalOccupantId(senderJid);
    if (occupantId == null) {
      return null;
    }
    final occupant = roomState.occupants[occupantId];
    if (occupant?.isPresent != true) {
      return null;
    }
    return occupant;
  }

  String? _realJidForExactPresentMucSender({
    required String chatJid,
    required String senderJid,
  }) {
    final occupant = _exactPresentMucOccupantForSender(
      chatJid: chatJid,
      senderJid: senderJid,
    );
  ...
  String? _liveMucSenderRealJidForMessageEvent(mox.MessageEvent event) {
    if (event.type != _messageTypeGroupchat ||
        _isArchivedOrOfflineMessage(event)) {
      return null;
    }
    return _realJidForExactPresentMucSender(
      chatJid: event.from.toBare().toString(),
      senderJid: event.from.toString(),
    );
  }

  String? _selfMucUserItemRealJid(mox.MessageEvent event) {
    if (event.type != _messageTypeGroupchat) {
      return null;
    }
    ...
    final roomState = roomStateFor(event.from.toBare().toString());
    if (roomState == null ||
        !roomState.hasPresentSelfOccupant ||
        !roomState.isSelfOccupantId(event.from.toString())) {
      return null;
    }
    final realJid = bareAddress(event.get<MucUserItemData>()?.jid)?.trim();
    if (realJid == null || realJid.isEmpty) {
      return null;
    }
    return sameNormalizedAddressValue(realJid, accountJid) ? accountJid : null;
  }
  ...
  bool _isSelfPinMutation(mox.MessageEvent event) {
    ...
    final realJid = _liveMucSenderRealJidForMessageEvent(event);
    if (realJid == null || realJid.isEmpty) {
      return false;
    }
  ...
  bool _isSelfArchivedGroupChatMessageEvent(mox.MessageEvent event) {
    ...
    if (!event.isFromMAM && _isSelfPinMutation(event)) {
      return true;
    }
    final accountJid = myJid;
    if (accountJid == null) {
    ...
    if (_selfMucUserItemRealJid(event) != null) {
      return true;
    }
    final realJid = _liveMucSenderRealJidForMessageEvent(event);
  ...
  String? _senderRealJidForInboundMessageEvent(
    ...
    if (chatType != ChatType.groupChat) {
      return super._senderRealJidForInboundMessageEvent(
        event,
        chatType: chatType,
      );
    }
    return _selfMucUserItemRealJid(event) ??
        _liveMucSenderRealJidForMessageEvent(event);
  }
  ...
  String? _senderRealJidForInboundMutationEvent(
    ...
    if (!isGroupChat) {
      return super._senderRealJidForInboundMutationEvent(
        event,
        isGroupChat: isGroupChat,
      );
    }
  ...
  ({String senderJid, bool identityVerified}) _resolveReactionSenderIdentity({
    ...
    if (normalizedSenderRealJid != null && normalizedSenderRealJid.isNotEmpty) {
      if (accountJid != null &&
          sameNormalizedAddressValue(normalizedSenderRealJid, accountJid)) {
        return (senderJid: accountJid, identityVerified: true);
      }
      if (!isArchived) {
        return (senderJid: normalizedSenderRealJid, identityVerified: true);
      }
    }
  ...
  Set<String> _reactionSenderAliases({
    ...
    final aliases = super._reactionSenderAliases(
    ...
    final roomJid = bareAddress(rawSenderJid)?.trim();
    if (roomJid == null || roomJid.isEmpty) {
      return aliases;
    }
    final realJid = _realJidForExactPresentMucSender(
      chatJid: roomJid,
      senderJid: rawSenderJid,
    );
    if (realJid == null || realJid.isEmpty || realJid == senderJid) {
      return aliases;
    }
    return Set<String>.unmodifiable({...aliases, realJid});
  }
  ...
  RegisteredStateKey _pendingInviteesStorageKey(String roomJid) =>
      XmppStateStore.registerKey(
        '$_mucPendingInviteesStorageKeyPrefix${_roomKey(roomJid)}',
      );
  ...
  Future<void> _persistRoomMemberSnapshot(RoomState room) async {
    ...
    final existing = await _loadPersistedRoomMemberSnapshot(room.roomJid);
    final merged = _mergeRoomMemberSnapshots(
      existing: existing,
      current: snapshot,
    );
    await _dbOp<XmppStateStore>(
      (ss) => ss.write(
        ...
        value: merged.map((entry) => entry.toJson()).toList(growable: false),
      ),
      awaitDatabase: true,
    );
  }

  Future<void> _replaceRoomMemberSnapshot(RoomState room) async {
    final snapshot = _snapshotMembersForRoom(room).toList(growable: false);
    if (snapshot.isEmpty) {
      await _clearPersistedRoomMemberSnapshot(room.roomJid);
      return;
    }
    await _dbOp<XmppStateStore>(
      (ss) => ss.write(
  ...
  List<_PersistedRoomMember> _mergeRoomMemberSnapshots({
    required List<_PersistedRoomMember> existing,
    required List<_PersistedRoomMember> current,
  }) {
    if (existing.isEmpty) {
      return List<_PersistedRoomMember>.unmodifiable(current);
    }
    final merged = <_PersistedRoomMember>[...current];
    for (final stored in existing) {
      final index = merged.indexWhere(
        (entry) => _samePersistedRoomMember(entry, stored),
      );
      if (index == -1) {
        merged.add(stored);
      } else {
        merged[index] = merged[index]._mergeFallback(stored);
      }
      if (merged.length >= _roomMemberSnapshotLimit) {
        break;
      }
    }
    return List<_PersistedRoomMember>.unmodifiable(
      merged.take(_roomMemberSnapshotLimit),
    );
  }

  bool _samePersistedRoomMember(
    _PersistedRoomMember left,
    _PersistedRoomMember right,
  ) {
    final leftRealJid = _normalizeBareJid(left.realJid);
    final rightRealJid = _normalizeBareJid(right.realJid);
    if (leftRealJid != null &&
        rightRealJid != null &&
        leftRealJid == rightRealJid) {
      return true;
    }
    final leftOccupantId = left.occupantId?.trim();
    final rightOccupantId = right.occupantId?.trim();
    if (leftOccupantId != null &&
        leftOccupantId.isNotEmpty &&
        rightOccupantId != null &&
        rightOccupantId.isNotEmpty &&
        leftOccupantId == rightOccupantId) {
      return true;
    }
    final leftNick = left.nick.trim().toLowerCase();
    final rightNick = right.nick.trim().toLowerCase();
    return leftNick.isNotEmpty && leftNick == rightNick;
  }
  ...
  Future<Set<String>> _loadPersistedPendingInvitees(String roomJid) async {
    final stored = await _dbOpReturning<XmppStateStore, Object?>(
      (ss) => ss.read(key: _pendingInviteesStorageKey(roomJid)),
    );
    if (stored is! List) {
      return const <String>{};
    }
    ...
    for (final value in stored) {
      ...
      final normalized = _normalizeBareJid(value);
      if (normalized == null || normalized.isEmpty) continue;
      invitees.add(normalized);
    }
    return Set<String>.unmodifiable(invitees);
  }

  Future<void> _persistPendingInvitees(
    String roomJid,
    Set<String> invitees,
  ) async {
    if (invitees.isEmpty) {
      await _dbOp<XmppStateStore>(
        (ss) => ss.delete(key: _pendingInviteesStorageKey(roomJid)),
        awaitDatabase: true,
      );
      return;
    }
    await _dbOp<XmppStateStore>(
      (ss) => ss.write(
        key: _pendingInviteesStorageKey(roomJid),
        value: invitees.toList(growable: false),
      ),
      awaitDatabase: true,
    );
  }

  Future<void> _removePersistedPendingInvitee({
    required String roomJid,
    required String inviteeJid,
  }) async {
    final normalizedInvitee = _normalizeBareJid(inviteeJid);
    if (normalizedInvitee == null || normalizedInvitee.isEmpty) {
      return;
    }
    final persisted = await _loadPersistedPendingInvitees(roomJid);
    if (!persisted.contains(normalizedInvitee)) {
      return;
    }
    final updated = Set<String>.of(persisted)..remove(normalizedInvitee);
    await _persistPendingInvitees(roomJid, updated);
  }

  Future<void> _restorePersistedPendingInvitees(String roomJid) async {
    final pendingInvitees = await _loadPersistedPendingInvitees(roomJid);
    ...
    final room = existing.copyWith(pendingInviteeJids: pendingInvitees);
    if (identical(room, existing)) {
      return;
    }
    _publishRoomState(roomKey: key, room: room, persistSnapshot: false);
  }

  RoomState? _applyPendingInviteeUpdate({
    required String roomJid,
    required String inviteeJid,
    ...
    final existing =
        _roomStates[key] ?? RoomState(roomJid: key, occupants: const {});
    final room = pending
        ? existing.withPendingInvitee(normalizedInvitee)
        : existing.withoutPendingInvitee(normalizedInvitee);
    if (identical(room, existing)) {
      return null;
    }
    _publishRoomState(roomKey: key, room: room, persistSnapshot: false);
  ...
  Future<void> _markRoomInviteePending({
    required String roomJid,
    required String inviteeJid,
  }) async {
    final room = _applyPendingInviteeUpdate(
      roomJid: roomJid,
    ...
    await _persistPendingInvitees(room.roomJid, room.pendingInviteeJids);
  }

  Future<void> _clearRoomPendingInvitee({
    required String roomJid,
    required String inviteeJid,
  }) async {
    final room = _applyPendingInviteeUpdate(
    ...
    await _removePersistedPendingInvitee(
      roomJid: roomJid,
      inviteeJid: inviteeJid,
    );
  }
  ...
  Future<void> _applyMucInviteLifecycleToRoomState(Message message) async {
    ...
    final data = message.pseudoMessageData;
    if (data == null) {
      return;
    }
    final roomJid = data['roomJid'] as String?;
    final inviteeJid = data['invitee'] as String?;
    if (roomJid == null || inviteeJid == null) {
      return;
    }
    await _clearRoomPendingInvitee(roomJid: roomJid, inviteeJid: inviteeJid);
  }
  ...
  void _publishRoomState({
    ...
    final suppressSnapshot =
        _roomSessionForKey(roomKey)?.suppressSnapshotPersistence == true;
    if (!persistSnapshot || suppressSnapshot) {
      return;
    }
    fireAndForget(
      () => replaceSnapshot
          ? _replaceRoomMemberSnapshot(room)
  ...
  void configureEventHandlers(EventManager<mox.XmppEvent> manager) {
    ...
    manager
      ...
      ..registerHandler<mox.MemberChangedNickEvent>((event) async {
        await _handleMemberNickChanged(
          event.roomJid,
          event.oldNick,
          event.newNick,
        );
      ...
      ..registerHandler<RoomVCardAvatarUpdatedEvent>((event) async {
        if (event.jid.resource.isNotEmpty) return;
  ...
  void retainRoomPresence(String roomJid) {
    final key = _roomKey(roomJid);
    final session = _ensureRoomSessionForKey(key);
    session.cancelIdleLeaveTimer();
    session.presenceRetainCount++;
  }

  void releaseRoomPresence(String roomJid) {
    final key = _roomKey(roomJid);
    final session = _roomSessionForKey(key);
    if (session == null) return;
    if (session.presenceRetainCount > 0) {
      session.presenceRetainCount--;
    }
    if (session.presenceRetainCount > 0) return;
    if (_roomExplicitlyLeft(key) || _roomHasLeft(key)) return;
    session.cancelIdleLeaveTimer();
    session.idleLeaveTimer = Timer(_mucIdleLeaveDelay, () {
      _startIdleLeaveTask(key);
    });
  }

  void _startIdleLeaveTask(String roomJid) {
    final key = _roomKey(roomJid);
    final session = _roomSessionForKey(key);
    if (session == null) return;
    final existing = session.idleLeaveFuture;
    if (existing != null) return;
    session.idleLeaveFuture = _runTrackedIdleLeaveTask(key, session);
  }

  Future<void> _runTrackedIdleLeaveTask(
    String roomJid,
    _MucRoomSession session,
  ) async {
    try {
      await _leaveIdleRoomPresence(roomJid);
    } on Exception catch (error, stackTrace) {
      _mucLog.fine(_mucIdleLeaveFailedLog, error, stackTrace);
    } finally {
      if (identical(_roomSessionForKey(roomJid), session)) {
        session.idleLeaveFuture = null;
      }
    }
  }

  Future<void> _leaveIdleRoomPresence(String roomJid) async {
    final key = _roomKey(roomJid);
    final session = _roomSessionForKey(key);
    if (session == null) return;
    ...
    try {
      await manager
          .leaveRoom(mox.JID.fromString(key))
          .timeout(_roomActionTimeout);
    } on Exception catch (error, stackTrace) {
      _mucLog.fine(_mucIdleLeaveFailedLog, error, stackTrace);
      return;
    }
    final currentSession = _roomSessionForKey(key);
    if (currentSession == null) return;
    if (_roomExplicitlyLeft(key) || _roomStates[key]?.hasTerminalExit == true) {
      return;
    }
    if (_roomPresenceRetained(key) && !_roomExplicitlyLeft(key)) {
      await _markRoomSelfPresenceUnavailable(
        key,
        statusCodes: _emptyStatusCodes,
      );
      await _rejoinRetainedRoom(key);
      return;
    }
    await _markRoomLeft(
      key,
      statusCodes: _emptyStatusCodes,
      preserveOccupants: true,
  ...
  Future<bool> _hasMucPresenceForSend({
    ...
    return _hasCurrentMucPresenceForSend(
      roomJid: roomJid,
  ...
  MucActorIdentity _outboundMucActorIdentity({
    ...
    final accountBareJid = bareAddress(accountJid) ?? accountJid;
  ...
  Future<void> _ensureMucJoinForSend({required String roomJid}) async {
    ...
    await _awaitIdleLeaveTaskIfActive(normalizedRoom);
  ...
  Future<void> _logMucSendDiagnostics({
    ...
    final managerCacheJoined = managerState?.joined == true;
    ...
    _mucLog.fine(
      ...
      'managerCacheJoined=$managerCacheJoined '
  ...
  void _completeTimedOutJoinAttempt(String roomJid) {
    final key = _roomKey(roomJid);
    final attemptId = _joinAttemptIdForKey(key);
    final shouldEmitOperation = _consumeJoinOperationTracked(key);
    if (attemptId != null) {
      if (shouldEmitOperation) {
        emitXmppOperation(_mucJoinEndEvent(isSuccess: false));
      }
      _logJoinEvent(
        message: _mucJoinCompletedLog,
        attemptId: attemptId,
  ...
  bool _roomReadyFromSelfPresence(String roomJid) {
    final room = _roomStates[_roomKey(roomJid)];
    return room?.hasSelfPresence == true &&
    ...
    try {
      ...
      final result = await manager
          ...
          .timeout(
            ...
            onTimeout: () {
              final roomReady = _roomReadyFromSelfPresence(roomJid);
              _logJoinEvent(
                message: roomReady
                    ? _mucJoinManagerJoinTimeoutLog
                    : _mucJoinManagerTimeoutBeforeReadyLog,
                attemptId: joinAttemptId,
                hasSelfPresence: roomReady,
              );
              return const moxlib.Result<bool, mox.MUCError>(true);
            },
          );
      if (result.isType<mox.MUCError>()) {
        if (_roomReadyFromSelfPresence(roomJid)) {
          _logJoinEvent(
            message: _mucJoinManagerErrorAfterReadyLog,
            attemptId: joinAttemptId,
  ...
  Future<void> _markRoomLeft(
    ...
    final key = _roomKey(roomJid);
    _roomSessionForKey(key)?.cancelIdleLeaveTimer();
    _clearRoomSelfPresenceReadiness(key);
    ...
    _publishRoomState(
      roomKey: key,
      room: room,
      persistSnapshot: persistSnapshot && !isDestroyed,
    );
  ...
  Future<void> _markRoomSelfPresenceUnavailable(
    String roomJid, {
    Set<String>? statusCodes,
    String? reason,
  ...
  void _applySelfPresenceStatus({
    ...
    _publishRoomState(roomKey: key, room: updated, persistSnapshot: false);
  }
  ...
  Future<RoomState> warmRoomFromHistory({
    ...
    await _restorePersistedPendingInvitees(roomJid);
    await _restorePersistedRoomMembers(roomJid);
    await _restoreRoomMembersFromStoredMessages(roomJid: key, limit: limit);
    return roomStateFor(roomJid) ??
        RoomState(roomJid: _roomKey(roomJid), occupants: const {});
  }

  Future<RoomState> prepareRoomMemberState({required String roomJid}) =>
      warmRoomFromHistory(roomJid: roomJid);

  Future<void> _restoreRoomMembersFromStoredMessages({
    required String roomJid,
    required int limit,
  }) async {
    if (limit <= 0 || !isDatabaseReady) {
      return;
    }
    ...
    for (final message in messages) {
      restored =
          _restoreRoomMemberFromStoredMessage(
            roomJid: roomJid,
            message: message,
          ) ||
          restored;
    ...
    final room = roomStateFor(roomJid);
    if (room != null) {
      _publishRoomState(roomKey: roomJid, room: room);
    }
  }

  bool _restoreRoomMemberFromStoredMessage({
    required String roomJid,
    required Message message,
  }) {
    final key = _roomKey(roomJid);
    if (_normalizeBareJid(message.chatJid) != key) {
      return false;
    }
    final senderJid = message.senderJid.trim();
    if (_normalizeBareJid(senderJid) != key) {
      return false;
    }
    final senderNick = addressResourcePart(senderJid)?.trim();
    final storedOccupantId = message.occupantID?.trim();
    final normalizedRealJid = _normalizeBareJid(message.senderRealJid);
    final existing =
        _roomStates[key] ?? RoomState(roomJid: key, occupants: const {});
    final matchedOccupant = _storedMessageOccupantMatch(
      room: existing,
      occupantId: storedOccupantId,
      senderNick: senderNick,
      realJid: normalizedRealJid,
    );
    final occupantId =
        matchedOccupant?.occupantId ??
        (storedOccupantId?.isNotEmpty == true
            ? storedOccupantId!
            : senderNick?.isNotEmpty == true
            ? '$key/$senderNick'
            : null);
    if (occupantId == null || occupantId.isEmpty) {
      return false;
    }
    final nick = senderNick?.isNotEmpty == true
        ? senderNick!
        : matchedOccupant?.nick.trim().isNotEmpty == true
        ? matchedOccupant!.nick
        : normalizedRealJid == null
        ? null
        : existing.fallbackNickForAffiliationJid(normalizedRealJid);
  ...
  Occupant? _storedMessageOccupantMatch({
    required RoomState room,
    ...
    if (realJid != null && realJid.isNotEmpty) {
      final byRealJid = room.occupantForRealJid(realJid);
      if (byRealJid != null) {
        return byRealJid;
      }
    }
    ...
    if (trimmedOccupantId != null && trimmedOccupantId.isNotEmpty) {
      final canonical = room.canonicalOccupantId(trimmedOccupantId);
      if (canonical != null) {
        return room.occupants[canonical];
      }
    }
    final trimmedSenderNick = senderNick?.trim();
    if (trimmedSenderNick != null && trimmedSenderNick.isNotEmpty) {
      return room.occupantForNick(trimmedSenderNick, preferRealJid: true);
    }
    return null;
  }
  ...
  Future<void> joinRoom({
    ...
    try {
      ...
      _clearRoomSelfPresenceReadiness(normalizedRoom);
      ...
      if (roomState?.isReadyForMessaging != true) {
        ...
        _completeTimedOutJoinAttempt(normalizedRoom);
        _markRoomWaitingForLateSelfPresence(normalizedRoom);
      } else {
  ...
  Future<void> ensureJoined({
    ...
    if (!forceRejoin) {
      await _awaitIdleLeaveTaskIfActive(key);
      if (_roomExplicitlyLeft(key)) return;
      if (_roomHasLeft(key) && !allowRejoin) return;
      if (_roomStates[key]?.hasTerminalExit == true) {
        return;
      }
    }
  ...
  Future<void> inviteUserToRoom({
    ...
    final shouldGrantMembership =
        canManageMembership || await _roomRequiresMembership(normalizedRoom);
    await _markRoomInviteePending(
      roomJid: normalizedRoom,
      inviteeJid: normalizedInvitee,
    );
    try {
      if (shouldGrantMembership) {
      ...
      await _clearRoomPendingInvitee(
        roomJid: normalizedRoom,
        inviteeJid: normalizedInvitee,
      );
      rethrow;
  ...
  Future<List<MucAffiliationEntry>> fetchRoomAffiliations({
    ...
    final entries = query
        ...
        .toList(growable: false);
    final selfAffiliation =
        roomStateFor(normalizedRoom)?.myAffiliation ?? OccupantAffiliation.none;
    final pruneMissing = selfAffiliation.canAuthoritativelyRefreshAffiliation(
      affiliation,
    );
    _mucLog.fine(
      'MUC affiliation query result. '
      'affiliation=${affiliation.xmlValue} '
      'self_affiliation=${selfAffiliation.xmlValue} '
  ...
  Future<void> leaveRoom(String roomJid) async {
    if (_connection.getManager<MUCManager>() case final manager?) {
      final normalizedRoom = _roomKey(roomJid);
      final wasExplicitlyLeft = _roomExplicitlyLeft(normalizedRoom);
      _setRoomExplicitlyLeft(normalizedRoom, true);
      try {
        await manager
            .leaveRoom(mox.JID.fromString(normalizedRoom))
            .timeout(_roomActionTimeout);
      } on Exception {
        _setRoomExplicitlyLeft(normalizedRoom, wasExplicitlyLeft);
        rethrow;
      }
      _forgetRoomNickname(roomJid: normalizedRoom);
      _forgetRoomPassword(roomJid: normalizedRoom);
      await _markRoomLeft(normalizedRoom, statusCodes: const <String>{});
  ...
  Future<void> acceptRoomInvite({
    ...
    if (_hasCurrentMucPresenceForSend(roomJid: roomJid) &&
        !hasLeftRoom(roomJid)) {
      await _sendInviteAcceptanceIfAvailable(
        roomJid: roomJid,
        roomName: roomName,
  ...
  Future<void> _sendInviteAcceptanceIfAvailable({
    required String roomJid,
    required String? roomName,
    required String? inviteToken,
    required String? inviterJid,
    required String? inviteeJid,
  }) async {
    final token = inviteToken?.trim();
    final inviterBare = bareAddress(inviterJid);
    if (token == null || token.isEmpty || inviterBare == null) {
      return;
    }
    try {
      final myBare = _myJid?.toBare().toString();
      if (myBare == null) throw XmppMessageException();
      final inviteeBare = bareAddress(inviteeJid) ?? myBare;
      final trimmedRoomJid = roomJid.trim();
      if (trimmedRoomJid.isEmpty || !sameBareAddress(inviteeBare, myBare)) {
        throw XmppMessageException();
      }
      final resolvedRoomName = switch (roomName?.trim()) {
        final String value when value.isNotEmpty => value,
        _ => (await _dbOpReturning<XmppDatabase, Chat?>(
          (db) => db.getChat(trimmedRoomJid),
        ))?.title,
      };
      final payload = <String, dynamic>{
        'roomJid': trimmedRoomJid,
        'token': token,
        'inviter': inviterBare,
        'invitee': myBare,
      ...
      final resolvedDisplayBody = resolvedRoomName?.isNotEmpty == true
          ? 'Invite accepted for $resolvedRoomName'
          : 'Invite accepted';
      ...
      final stanza = mox.MessageEvent(
        parseJidOrThrow(myBare),
        parseJidOrThrow(inviterBare),
        false,
        mox.TypedMap<mox.StanzaHandlerExtension>.fromList([
  ...
  Future<void> _handleSelfPresence(MucSelfPresenceEvent event) async {
    ...
    if (_isRetainedStaleSelfUnavailable(roomJid: roomJid, event: event)) {
      await _markRoomSelfPresenceUnavailable(
        roomJid,
        statusCodes: event.statusCodes,
        reason: event.reason,
      );
      await _rejoinRetainedRoom(roomJid);
      return;
    }
  ...
  Future<void> _handleMemberNickChanged(
    mox.JID roomJid,
    String oldNick,
    String newNick,
  ) async {
  ...
  void removeOccupant({required String roomJid, required String occupantId}) {
    ...
    final existing = _roomStates[key];
    if (existing == null) return;
    final resolvedOccupantId = existing.canonicalOccupantId(occupantId);
    final occupant = resolvedOccupantId == null
        ? null
        : existing.occupants[resolvedOccupantId];
    final room = occupant?.affiliation.isPersistentRoomMember == true
        ? existing.withOccupantUnavailable(occupantId)
        : existing.withoutOccupant(occupantId);
  ...
  void _applyAffiliationEntries({
    ...
    _publishRoomState(roomKey: key, room: room, replaceSnapshot: pruneMissing);
  }
  ...
  RoomState _upsertOccupant({
    ...
    var clearedPendingInvitee = false;
    final shouldPersistSnapshot =
        current == null ||
        base.occupantId != next.occupantId ||
        base.nick != next.nick ||
        base.realJid != next.realJid ||
        base.affiliation != next.affiliation ||
        base.role != next.role ||
        (matchedOccupantId != null && matchedOccupantId != resolvedOccupantId);
    final isNickMatch =
    ...
    final joinedInviteeJid = next.isPresent ? next.normalizedBareRealJid : null;
    if (joinedInviteeJid != null) {
      final withoutPendingInvitee = room.withoutPendingInvitee(
        joinedInviteeJid,
      );
      clearedPendingInvitee = !identical(withoutPendingInvitee, room);
      room = withoutPendingInvitee;
    }
    ...
    _publishRoomState(
      roomKey: key,
      room: room,
      persistSnapshot: persistSnapshot && shouldPersistSnapshot,
    );
    if (clearedPendingInvitee) {
      fireAndForget(
        () => _persistPendingInvitees(key, room.pendingInviteeJids),
        operationName: 'MucService.persistPendingInvitees',
      );
    }
    return room;
  }
  ...
  Future<void> seedDummyRoomData(String roomJid) async {
    ...
    for (var index = 0; index < dummyMembers.length; index++) {
      ...
      final message = Message(
        stanzaID: stanzaID,
        senderJid: senderJid,
        senderRealJid: '${member.id}@example.test',
        chatJid: roomJid,
        body: member.message,
