...
class MamStreamManagementGuard extends mox.XmppManagerBase {
  ...
  List<mox.StanzaHandler> getIncomingPreStanzaHandlers() => [
    mox.StanzaHandler(priority: 9998, callback: _onIncomingPreStanza),
  ];
...
class XmppTransportChatMarkerEvent extends mox.XmppEvent {
  ...
  bool get isArchived => isFromMAM || isDelayed;
}
    ...
    try {
      getAttributes().sendEvent(
        XmppTransportChatMarkerEvent(
          ...
          isDelayed: state.extensions.get<mox.DelayedDeliveryData>() != null,
...
final class _PendingReadMarker {
  const _PendingReadMarker({
    required this.to,
...
final class _PendingInboundAcknowledgement {
  ...
  final String stanzaId;
  final bool chatMarker;
  final bool deliveryReceipt;

  bool get hasWork => chatMarker || deliveryReceipt;

  _PendingInboundAcknowledgement copyWith({
    bool? chatMarker,
    bool? deliveryReceipt,
  }) {
    return _PendingInboundAcknowledgement(
      target: target,
      chatJid: chatJid,
      stanzaId: stanzaId,
      chatMarker: chatMarker ?? this.chatMarker,
      deliveryReceipt: deliveryReceipt ?? this.deliveryReceipt,
    );
  }

  _PendingInboundAcknowledgement merge(_PendingInboundAcknowledgement pending) {
    return _PendingInboundAcknowledgement(
      target: target,
      chatJid: chatJid,
      stanzaId: stanzaId,
      chatMarker: chatMarker || pending.chatMarker,
      deliveryReceipt: deliveryReceipt || pending.deliveryReceipt,
    );
  }
}
...
const String _pinArchiveBootstrapFailedLog =
    'Failed to bootstrap shared pinned messages from archive.';
const String _pinArchiveBootstrapAbortedLog =
    'Shared pinned message bootstrap aborted.';
...
extension _OutboundMessageFlagView on _OutboundMessageFlag {
  String get label => switch (this) {
    ...
    _OutboundMessageFlag.pinMutation => _outboundSummaryFlagPinMutation,
  };
}
...
final class _CalendarMamSession {
  bool inFlight = false;
  bool snapshotSeen = false;
  bool snapshotUnavailableNotified = false;
  bool pageHadProcessingFailure = false;
  bool runHadProcessingFailure = false;
  String? newestArchiveResumeId;

  void beginPage() {
    pageHadProcessingFailure = false;
  }

  void recordPageProcessingFailure() {
    pageHadProcessingFailure = true;
    runHadProcessingFailure = true;
  }

  void resetRun() {
    snapshotSeen = false;
    snapshotUnavailableNotified = false;
    pageHadProcessingFailure = false;
    runHadProcessingFailure = false;
    newestArchiveResumeId = null;
  }
}
...
enum CalendarMamOutcome {
  completed,
  skippedCoveredByGlobal,
  skippedUnsupported,
  skippedUnauthorized,
  skippedInFlight,
  incomplete,
  ...
  bool get isSuccessful => switch (this) {
    ...
    CalendarMamOutcome.skippedInFlight => true,
    CalendarMamOutcome.incomplete || CalendarMamOutcome.failed => false,
  };
}
...
const String _calendarReadOnlyTaskOwnersKeyName =
    'calendar_read_only_task_owners_v1';
...
final _calendarReadOnlyTaskOwnersKey = XmppStateStore.registerKey(
  _calendarReadOnlyTaskOwnersKeyName,
);
...
const String _calendarSyncWarningStorageKeyName = 'calendar_sync_warning_v1';
...
mixin MessageService on XmppBase, BaseStreamService, BlockingService {
  ...
  final Set<String> _pendingDraftPublishes = <String>{};
  final Set<String> _pendingDraftRetractions = <String>{};
  ...
  Future<void> updateDraftAutosaveEnabled({
    ...
    await _dbOp<XmppDatabase>(
      (db) => db.updateDraftAutosaveEnabled(id: id, enabled: enabled),
    );
  }
  ...
  Future<void> syncMessageCollectionsSnapshot() async {
    ...
    try {
      ...
      final localRecords = await _localMessageCollectionDefinitions(
        includeInactive: true,
      );
      final localRecordsByItemId = <String, MessageCollectionEntry>{
        for (final entry in localRecords)
          MessageCollectionRecordSyncPayload.itemIdFor(collectionId: entry.id):
              entry,
      };

      for (final remote in snapshot.items) {
        switch (remote) {
          case MessageCollectionRecordSyncPayload():
            if (SystemMessageCollection.isSystemId(remote.collectionId)) {
              continue;
            }
            final local = localRecordsByItemId.remove(remote.itemId);
            if (local == null) {
              await _applyMessageCollectionDefinitionSyncUpdate(remote);
              continue;
            }
            final syncDecision =
                _resolveMessageCollectionDefinitionSyncDecision(local, remote);
      ...
      for (final local in localRecordsByItemId.values) {
        await _publishMessageCollectionDefinitionSyncEntry(
          local,
          managerOverride: manager,
          managerReady: true,
  ...
  CalendarSyncWarning? takePendingCalendarSyncWarning() {
    final storage = _calendarSyncWarningStorage();
    if (storage == null) {
      return null;
    }
    final raw = storage.read(_calendarSyncWarningStorageKey);
    final warning = raw is String ? _calendarSyncWarningFromWire(raw) : null;
    if (warning != null) {
      unawaited(storage.delete(_calendarSyncWarningStorageKey));
    }
    return warning;
  }

  void registerCalendarSyncFlushCallback(Future<void> Function() callback) {
    _calendarSyncFlushCallbacks.add(callback);
  }

  void unregisterCalendarSyncFlushCallback(Future<void> Function() callback) {
    _calendarSyncFlushCallbacks.remove(callback);
  }
        ...
        .map((messages) {
          ...
          SafeLogging.profileTrace(
            ...
            fields: <String, Object?>{
              'chatHash': SafeLogging.profileFingerprint(jid.trim()),
  ...
  Future<void> purgeMessageHistory({bool awaitDatabase = true}) async {
    ...
    await _clearSharedPinArchiveBootstrapMarkers(
      chats.map((chat) => chat.remoteJid),
    );
  }
  ...
  Future<void> _hydrateDuplicatePayload({
    ...
    await _dbOp<XmppDatabase>((db) async {
      ...
      if (incoming.originID?.isNotEmpty == true) {
        existing = await db.getMessageByOriginID(
          incoming.originID!,
  ...
  RegisteredStateKey _pinArchiveBootstrapKeyFor(String chatJid) {
    final normalizedChat = _normalizePinChatJid(chatJid) ?? chatJid.trim();
    return _pinArchiveBootstrapKeyCache.putIfAbsent(
      normalizedChat,
      () => XmppStateStore.registerKey(
        '$_pinArchiveBootstrapKeyPrefix$normalizedChat',
      ),
    );
  }
  ...
  Future<void> _markSharedPinArchiveBootstrapComplete(String chatJid) async {
    await _dbOp<XmppStateStore>(
      (ss) => ss.write(
        key: _pinArchiveBootstrapKeyFor(chatJid),
  ...
  Future<void> _clearSharedPinArchiveBootstrapMarkers(
    Iterable<String> chatJids,
  ) async {
    final normalized = chatJids
        .map(_normalizePinChatJid)
        .whereType<String>()
        .toSet();
    if (normalized.isEmpty) {
      return;
  ...
  bool _readOnlyTaskOwnersLoaded = false;
  Future<void>? _readOnlyTaskOwnersLoad;
  final Map<
    ({
      String metadataId,
      String? stanzaId,
  ...
  final Map<String, _PendingReadMarker> _pendingReadMarkersByTarget = {};
  final Map<String, Map<String, _PendingInboundAcknowledgement>>
  _pendingInboundAcknowledgementsByTarget = {};
  final Object _receiptRetryBootstrapOperationKey = Object();
  ...
  Future<void> _publishMessageCollectionDefinitionSyncEntry(
    ...
    final collectionId = entry.id.trim();
    final itemId = MessageCollectionRecordSyncPayload.itemIdFor(
      collectionId: collectionId,
    );
    if (itemId.isEmpty ||
        entry.isSystem ||
  ...
  Future<void> _applyMessageCollectionDefinitionSyncUpdate(
    MessageCollectionRecordSyncPayload payload,
  ) async {
  ...
  _resolveMessageCollectionDefinitionSyncDecision(
    ...
    MessageCollectionRecordSyncPayload remote,
  ) {
  ...
  MessageCollectionRecordSyncPayload _buildMessageCollectionDefinitionPayload(
    MessageCollectionEntry entry,
  ) {
    return MessageCollectionRecordSyncPayload(
  ...
  Future<void> _flushPendingMessageCollectionSync({
    ...
    for (final itemId in pendingPublishes) {
      final recordEntry = localRecordsByItemId[itemId];
      if (recordEntry != null) {
        final published = await manager.publishEntry(
          _buildMessageCollectionDefinitionPayload(recordEntry),
        );
        if (published) {
      ...
      final membershipEntry = localByItemId[itemId];
  ...
  void configureEventHandlers(EventManager<mox.XmppEvent> manager) {
    ...
    registerBootstrapOperation(
      XmppBootstrapOperation(
        key: _calendarSyncFlushOperationName,
        priority: 2,
        triggers: const <XmppBootstrapTrigger>{
          XmppBootstrapTrigger.fullNegotiation,
          XmppBootstrapTrigger.resumedNegotiation,
        },
        operationName: _calendarSyncFlushOperationName,
        run: () async {
          await _flushPendingCalendarSync();
        },
      ),
    );
    registerBootstrapOperation(
    ...
    registerBootstrapOperation(
      XmppBootstrapOperation(
        key: _receiptRetryBootstrapOperationKey,
        priority: 2,
        triggers: const <XmppBootstrapTrigger>{
          XmppBootstrapTrigger.fullNegotiation,
          XmppBootstrapTrigger.resumedNegotiation,
        ...
        operationName:
            'MessageService.flushPendingReceiptRetriesOnNegotiations',
        run: () async {
          await _flushPendingReceiptRetries();
        },
      ),
    );
    manager
      ...
      ..registerHandler<MessageCollectionSyncUpdatedEvent>((event) async {
        ...
        switch (payload) {
          case MessageCollectionRecordSyncPayload():
            await _applyMessageCollectionDefinitionSyncUpdate(payload);
          case MessageCollectionSyncPayload():
      ...
      ..registerHandler<XmppTransportChatMarkerEvent>((event) async {
        if (await _applySelfDisplayedChatMarker(event)) {
          return;
        }
        await _applyPeerTransportChatMarker(event);
      })
  ...
  Future<String?> _resolveOutboundPinMessageId({
    ...
    final message = await _dbOpReturning<XmppDatabase, Message?>(
      (db) => db.getMessageByReferenceId(trimmedMessageId, chatJid: chatJid),
    );
    if (message == null) {
      return null;
    }
  ...
  void _trackOutboundMessageSummary({
    ...
    final List<_OutboundMessageFlag> flags = <_OutboundMessageFlag>[
      ...
      if (extensions.get<PinMessageMutationData>() != null)
        _OutboundMessageFlag.pinMutation,
    ];
  ...
  _OutboundPinMutation? _takeOutboundPinMutation(String? stanzaId) {
    final trimmedStanzaId = stanzaId?.trim();
  ...
  void _dropOutboundPinMutationByAction({
    ...
    _outboundPinMutationsByStanzaId.removeWhere(
      (_, mutation) =>
  ...
  bool _hasOutboundPinMutationInFlight({
    required String chatJid,
    required String messageStanzaId,
    required bool pinned,
    PinMessageMutationScope? scope,
  ...
  String? _normalizedOutboundQuotedStanzaId(String? value) {
    ...
    if (transportGroupId != null || attachmentOrder != null) {
      await _dbOp<XmppDatabase>((db) async {
        ...
        await db.addMessageAttachment(
          ...
          groupQuotedStanzaId: resolvedGroupQuotedStanzaId,
        );
      });
    }
    if (localOnly) {
  ...
  Future<Message?> loadMessageByReferenceId(
    ...
    return await _dbOpReturning<XmppDatabase, Message?>(
      (db) => db.getMessageByReferenceId(messageId, chatJid: chatJid),
    );
  }
  ...
  Future<void> markMessagesDisplayedLocally({
    ...
    await _dbOp<XmppDatabase>((db) async {
      if (db is XmppDrift) {
        path = 'driftUpdate';
        final query = db.update(db.messages)
          ..where(
            (tbl) =>
        ...
        path = 'database';
        updatedRows = messageList.any((message) => !message.displayed) ? 1 : 0;
  ...
  Future<bool> _canSendChatMarkers({
    ...
    if (sameNormalizedAddressValue(to, myJid)) return false;
    return true;
  }
  ...
  Future<bool> _sendReadMarkerNow(_PendingReadMarker pending) async {
    return _connection.sendChatMarker(
      to: pending.to,
  ...
  Future<void> _flushPendingReadMarkers() async {
    final entries = _pendingReadMarkersByTarget.entries.toList(growable: false);
    for (final entry in entries) {
      final pending = entry.value;
      if (!await _canSendChatMarkers(
        to: pending.to,
        chatType: pending.chatType,
      )) {
        _pendingReadMarkersByTarget.remove(entry.key);
        continue;
      }
      if (!await _sendReadMarkerNow(pending)) {
        continue;
      }
      if (identical(_pendingReadMarkersByTarget[entry.key], pending)) {
        _pendingReadMarkersByTarget.remove(entry.key);
      }
    }
  }
  ...
  bool _isSameDomainPeerJidForCapabilities(String jid) {
    final accountDomain = _myJid?.domain.trim().toLowerCase();
    if (accountDomain == null || accountDomain.isEmpty) {
      return false;
    }
    ...
    return parsed.domain.trim().toLowerCase() == accountDomain;
  }
  ...
  _PeerCapabilities _withCapabilityAssumptions({
    ...
    final assumedFeatures = _assumedCapabilityFeatures(jid);
    ...
    final supportsReceipts =
        capabilities.supportsReceipts ||
        assumedFeatures.contains(mox.deliveryXmlns);
    final features = <String>{
  ...
  Future<_PeerCapabilities> _capabilitiesFor(
    ...
    if (demoOfflineMode) {
      final capabilities = _withCapabilityAssumptions(
        jid: jid,
        capabilities: _demoPeerCapabilities(),
      );
      _capabilityCache[jid] = capabilities;
      return capabilities;
    }
    ...
    final Future<_PeerCapabilities> request = () async {
      ...
      if (result == null || result.isType<mox.StanzaError>()) {
        final fallback = _withCapabilityAssumptions(
          jid: jid,
  ...
  Future<void> _updateUnreadCountForChat(String chatJid) async {
    ...
    await _dbOp<XmppDatabase>((db) async {
      ...
      await db.repairUnreadCountForChat(
        normalizedChatJid,
        selfJid: myJid,
        emailSelfJid: chat.emailFromAddress,
      );
    });
  }
  ...
  Future<bool> _applyPeerTransportChatMarker(
    XmppTransportChatMarkerEvent event,
  ) async {
    if (!(event.isCarbon || event.isArchived) ||
    ...
    if (fromBare.isEmpty || sameNormalizedAddressValue(fromBare, accountJid)) {
      return false;
    }
    final toBare = event.to.toBare().toString();
    if (toBare.isNotEmpty && !sameNormalizedAddressValue(toBare, accountJid)) {
      return false;
    }
    ...
    await _recordObservedPeerCapabilities(jid: fromBare, supportsMarkers: true);
    return _applyPeerOutboundMessageStatus(
      chatJid: fromBare,
      id: referenceId,
      acked: true,
      received: _chatMarkerCountsAsReceived(event.type),
      displayed: _chatMarkerCountsAsDisplayed(event.type),
      applyThrough: true,
    );
  }
  ...
  Future<({bool acked, bool received})> _acknowledgeMessage(
    ...
    if (isMuc) {
      ...
      await _markIncomingMessageAcknowledged(
        stanzaId: localReferenceId,
        chatJid: chatJid,
      );
    ...
    final pending = _PendingInboundAcknowledgement(
      target: event.from.toString(),
      chatJid: chatJid,
      stanzaId: directStanzaId,
      chatMarker: markable,
      deliveryReceipt: deliveryReceiptRequested,
    );
    final sent = await _sendInboundAcknowledgementNow(pending);
    final acknowledged = sent.chatMarker || sent.deliveryReceipt;
    final unsent = pending.copyWith(
      chatMarker: pending.chatMarker && !sent.chatMarker,
      deliveryReceipt: pending.deliveryReceipt && !sent.deliveryReceipt,
    );
    if (unsent.hasWork) {
      _queuePendingInboundAcknowledgement(unsent);
    }
    if (!acknowledged) {
      return (acked: false, received: false);
    }
  ...
  Future<({bool chatMarker, bool deliveryReceipt})>
  _sendInboundAcknowledgementNow(_PendingInboundAcknowledgement pending) async {
    var sentChatMarker = false;
    if (pending.chatMarker) {
      sentChatMarker = await _connection.sendChatMarker(
        to: pending.target,
        stanzaID: pending.stanzaId,
    ...
    var sentDeliveryReceipt = false;
    if (pending.deliveryReceipt) {
      sentDeliveryReceipt = await _connection.sendMessage(
        mox.MessageEvent(
          _myJid!,
          mox.JID.fromString(pending.target),
          false,
          mox.TypedMap<mox.StanzaHandlerExtension>.fromList([
            mox.MessageDeliveryReceivedData(pending.stanzaId),
          ]),
    ...
    return (chatMarker: sentChatMarker, deliveryReceipt: sentDeliveryReceipt);
  }

  void _queuePendingInboundAcknowledgement(
    _PendingInboundAcknowledgement pending,
  ) {
    if (!pending.hasWork) return;
    final targetKey =
        normalizedBareAddressValue(pending.target) ?? pending.target;
    final normalizedTargetKey = targetKey.trim();
    if (normalizedTargetKey.isEmpty || pending.stanzaId.trim().isEmpty) {
      return;
    }
    final pendingById = _pendingInboundAcknowledgementsByTarget.putIfAbsent(
      normalizedTargetKey,
      () => <String, _PendingInboundAcknowledgement>{},
    );
    pendingById.update(
      pending.stanzaId,
      (current) => current.merge(pending),
      ifAbsent: () => pending,
    );
  }

  Future<void> _flushPendingInboundAcknowledgements() async {
    final targetEntries = _pendingInboundAcknowledgementsByTarget.entries
        .toList(growable: false);
    for (final targetEntry in targetEntries) {
      final pendingEntries = targetEntry.value.entries.toList(growable: false);
      for (final pendingEntry in pendingEntries) {
        ...
        final currentById =
            _pendingInboundAcknowledgementsByTarget[targetEntry.key];
        final current = currentById?[pending.stanzaId];
        ...
        if (unsent.hasWork) {
          currentById![pending.stanzaId] = unsent;
          continue;
        }
        currentById?.remove(pending.stanzaId);
        if (currentById?.isEmpty ?? false) {
          _pendingInboundAcknowledgementsByTarget.remove(targetEntry.key);
        }
      }
    }
  }

  Future<void> _markIncomingMessageAcknowledged({
    required String stanzaId,
    required String chatJid,
  }) async {
    await _dbOp<XmppDatabase>((db) async {
      await db.markMessageReceived(stanzaId, chatJid: chatJid);
      await db.markMessageAcked(stanzaId, chatJid: chatJid);
    });
  }

  Future<void> _flushPendingReceiptRetries() async {
    await _flushPendingReadMarkers();
    await _flushPendingInboundAcknowledgements();
  }
  ...
  Future<void> _reset() async {
    ...
    _mamGlobalMaxTimestamp = null;
    _calendarMamSessions.clear();
    _mamGlobalCompleteChatCalendarCandidates.clear();
    _readOnlyTaskOwnersByChat.clear();
    _readOnlyTaskOwnersLoaded = false;
    _readOnlyTaskOwnersLoad = null;
    _resetStableKeyCache();
    ...
    _liveRemoteCalendarSyncLimiter.reset();
  ...
  bool _handleCalendarSync(
    ...
    final senderJid = isGroupChat
        ? rawSenderJid
        : bareAddress(rawSenderJid) ?? rawSenderJid;
    final selfJid = bareAddress(myJid) ?? myJid;
    final chatJid = _calendarSyncChatJid(event, selfJid);
    final chatType = _calendarSyncChatType(event);
    final bool isSelfSender = _isSelfCalendarSyncSender(
      event: event,
      chatJid: chatJid,
      chatType: chatType,
      senderJid: senderJid,
    ...
    if (_isLiveRemoteCalendarSyncRateLimited(
      event,
      isSelfSender: isSelfSender,
    )) {
      return true;
  ...
  bool _isSelfCalendarSyncSender({
    ...
    return sameBareAddress(senderJid, selfJid);
  }

  bool _isLiveRemoteCalendarSyncRateLimited(
    mox.MessageEvent event, {
    required bool isSelfSender,
  }) {
    if (event.isFromMAM || isSelfSender) {
      return false;
    }
    if (_liveRemoteCalendarSyncLimiter.allowEvent()) {
      return false;
    }
    _log.warning(
      'Dropping live remote calendar sync message due to rate limits',
    );
    return true;
  }
  ...
  Future<void> _processQueuedCalendarSyncWork(_CalendarSyncWork work) async {
    ...
    if (!isSelfCalendar) {
      ...
      if (authorization != _CalendarSyncAuthorizationResult.allowed) {
    ...
    if (!isSelfCalendar &&
        await _isReadOnlyTaskSyncBlocked(
          syncMessage: syncMessage,
          event: event,
          chatJid: chatJid,
        )) {
  ...
  _CalendarSyncAuthorizationResult _chatCalendarSyncAuthorization({
    ...
    if (senderRole == null) {
      ...
      return _CalendarSyncAuthorizationResult.unresolved;
    }
    if (!senderRole.allows(requiredRole)) {
      _log.warning(_calendarSyncUnauthorizedLog);
      return _CalendarSyncAuthorizationResult.rejected;
    }
    return _CalendarSyncAuthorizationResult.allowed;
  }
  ...
  Future<bool> _isReadOnlyTaskSyncBlocked({
    required CalendarSyncMessage syncMessage,
    required mox.MessageEvent event,
    required String chatJid,
  }) async {
    ...
    final Map<String, String>? owners = _readOnlyTaskOwnersByChat[chatKey];
    ...
    return !_calendarReadOnlyOwnerMatchesSender(
      senderJid: _calendarSyncSenderIdentity(event),
      ownerJid: owner,
    );
  }

  Future<CalendarSyncMessage> _sanitizeChatCalendarSyncMessageForReadOnly({
    required CalendarSyncMessage syncMessage,
    required mox.MessageEvent event,
    required String chatJid,
  }) async {
    if (syncMessage.type != CalendarSyncType.full &&
        syncMessage.type != CalendarSyncType.snapshot) {
      return syncMessage;
    }
    final data = syncMessage.data;
    if (data == null) {
      return syncMessage;
    }
    await _ensureReadOnlyTaskOwnersLoaded();
    ...
    try {
      final remoteModel = CalendarModel.fromJson(data);
      ...
      return syncMessage.copyWith(
        ...
        snapshotChecksum: syncMessage.type == CalendarSyncType.snapshot
            ? checksum
            : syncMessage.snapshotChecksum,
      );
    } on Exception catch (error, stackTrace) {
      _log.warning(
        'Failed to inspect calendar snapshot for read-only task ownership.',
        error,
        stackTrace,
      );
      return syncMessage;
  ...
  bool _isReadOnlyTaskOwnerMismatch({
    ...
    final owner = owners[taskId]?.trim();
  ...
  String _calendarSyncSenderIdentity(mox.MessageEvent event) {
    if (event.type == _messageTypeGroupchat) {
  ...
  Future<void> _rememberReadOnlyTaskShare(Message message) async {
    ...
    final String? chatKey = _readOnlyTaskChatKey(message.chatJid);
    if (chatKey == null) {
      return;
    }
    ...
    if (owners[taskId] == owner) {
      return;
    }
    owners[taskId] = owner;
    await _writeReadOnlyTaskOwners();
  }

  String? _readOnlyTaskChatKey(String chatJid) {
    final normalized = bareAddress(chatJid) ?? chatJid.trim().toLowerCase();
    return normalized.isEmpty ? null : normalized;
  }

  Future<void> _ensureReadOnlyTaskOwnersLoaded() async {
    if (_readOnlyTaskOwnersLoaded) {
      return;
    }
    final pendingLoad = _readOnlyTaskOwnersLoad;
    if (pendingLoad != null) {
      await pendingLoad;
      return;
    }
    final load = _loadReadOnlyTaskOwners();
    _readOnlyTaskOwnersLoad = load;
    try {
      await load;
    } finally {
      _readOnlyTaskOwnersLoad = null;
  ...
  Future<void> _loadReadOnlyTaskOwners() async {
    final raw = await _dbOpReturning<XmppStateStore, String?>(
      (ss) => ss.read(key: _calendarReadOnlyTaskOwnersKey) as String?,
    );
    final trimmed = raw?.trim();
    if (trimmed != null && trimmed.isNotEmpty) {
      try {
        ...
        if (decoded is Map) {
          _readOnlyTaskOwnersByChat
            ..clear()
            ..addAll(_decodeReadOnlyTaskOwners(decoded));
        }
      } on FormatException catch (error, stackTrace) {
        _log.warning(
          'Stored read-only task owner map is invalid.',
          error,
          stackTrace,
        );
      }
    }
    _readOnlyTaskOwnersLoaded = true;
  }

  Map<String, Map<String, String>> _decodeReadOnlyTaskOwners(Map decoded) {
    final ownersByChat = <String, Map<String, String>>{};
    for (final chatEntry in decoded.entries) {
      final chatKey = _readOnlyTaskChatKey(chatEntry.key.toString());
      final rawTasks = chatEntry.value;
      if (chatKey == null || rawTasks is! Map) {
        continue;
      }
      final owners = <String, String>{};
      for (final taskEntry in rawTasks.entries) {
        final taskId = taskEntry.key.toString().trim();
        final owner = taskEntry.value.toString().trim();
        if (taskId.isEmpty || owner.isEmpty) {
          continue;
        }
        owners[taskId] = owner;
      }
      if (owners.isNotEmpty) {
        ownersByChat[chatKey] = owners;
      }
    }
    return ownersByChat;
  }

  Future<void> _writeReadOnlyTaskOwners() async {
    final payload = jsonEncode(_readOnlyTaskOwnersByChat);
    await _dbOp<XmppStateStore>(
      (ss) => ss.write(key: _calendarReadOnlyTaskOwnersKey, value: payload),
      awaitDatabase: true,
    );
  }
  ...
  Future<bool> _handleCalendarSnapshot(
    CalendarSyncMessage syncMessage,
    mox.MessageEvent event, {
    FileMetadataData? metadata,
    required bool isSelfCalendar,
    required bool allowRemoteDownload,
    required bool allowSelfDownloadOverrides,
    ...
    if (!hasUrl && metadata == null) {
      ...
      await _maybeNotifySnapshotUnavailable(
        event,
        isSelfCalendar: isSelfCalendar,
        mamSession: mamSession,
      );
      return false;
    }

    try {
      final bool? allowSelfOverride = allowSelfDownloadOverrides
          ? allowSelfDownloadOverrides
          : null;
  ...
  Future<void> _maybeNotifySnapshotUnavailable(
    ...
    if (!mamSession.inFlight && !event.isFromMAM) {
      return;
    }
    if (mamSession.snapshotUnavailableNotified) {
      return;
    }
    mamSession.snapshotUnavailableNotified = true;
  ...
  Future<void> _emitCalendarSnapshotWarning() async {
    ...
    await _emitCalendarSyncWarning(warning);
  }

  Future<void> _emitCalendarArchiveIncompleteWarning() async {
    const warning = CalendarSyncWarning(
      type: CalendarSyncWarningType.archiveIncomplete,
    );
    await _emitCalendarSyncWarning(warning);
  }

  Future<void> _emitCalendarSyncWarning(CalendarSyncWarning warning) async {
    if (!_calendarSyncWarningController.hasListener) {
      _log.info('No calendar sync warning listeners registered');
      await _storePendingCalendarSyncWarning(warning);
      return;
    }
    try {
  ...
  Future<void> _storePendingCalendarSyncWarning(
    CalendarSyncWarning warning,
  ) async {
    final storage = _calendarSyncWarningStorage();
    if (storage == null) {
      return;
    ...
    try {
      await storage.write(_calendarSyncWarningStorageKey, warning.type.name);
    } on Exception catch (error, stackTrace) {
      _log.fine('Failed to persist calendar sync warning.', error, stackTrace);
    }
  }

  Storage? _calendarSyncWarningStorage() {
    try {
  ...
  String get _calendarSyncWarningStorageKey =>
      '$authStoragePrefix$_calendarSyncWarningStorageKeyName';

  CalendarSyncWarning? _calendarSyncWarningFromWire(String value) {
    for (final type in CalendarSyncWarningType.values) {
      if (type.name == value) {
        return CalendarSyncWarning(type: type);
      }
    }
    return null;
  }
  ...
  ChatCalendarSyncCoordinator _directChatCalendarSyncCoordinator() {
    return _serviceChatCalendarSyncCoordinator ??= ChatCalendarSyncCoordinator(
      storage: ChatCalendarStorage(storage: HydratedBloc.storage),
      sendMessage:
          ({
            required String jid,
            required CalendarSyncOutbound outbound,
  ...
  Future<bool> _processChatCalendarSyncDirect(
    ...
    try {
      ...
      _log.warning(
        'Chat calendar storage unavailable for sync.',
        error,
        stackTrace,
      );
    } catch (error, stackTrace) {
  ...
  Future<void> _flushPendingCalendarSync() async {
    final callbacks = _calendarSyncFlushCallbacks.toList(growable: false);
    ...
    if (chatCoordinator != null) {
      try {
        await chatCoordinator.flushPending();
      } on Exception catch (error, stackTrace) {
        _log.warning(
          'Direct chat calendar sync flush failed.',
          error,
          stackTrace,
        );
      }
  ...
  Future<void> _recordRejectedCalendarEnvelope({
    required CalendarSyncMessage syncMessage,
    required mox.MessageEvent event,
    required bool isSelfCalendar,
    required String chatJid,
  ...
  Future<void> _recordCalendarMamEnvelopeResult({
    ...
    final handled = _calendarSyncStateCoversInbound(
      inbound: inbound,
      isSelfCalendar: isSelfCalendar,
      chatJid: chatJid,
    );
  ...
  Future<void> _recordCalendarMamEnvelopeFailure({
    required _CalendarMamSession mamSession,
    required mox.MessageEvent event,
    required bool isSelfCalendar,
    required String chatJid,
    ...
    if (mamSession.inFlight) {
      mamSession.recordPageProcessingFailure();
    }
    if (!_mamGlobalSyncInFlight || mamSession.inFlight) {
      return;
    }
    _mamGlobalCalendarProcessingFailed = true;
    if (isSelfCalendar) {
    ...
    final normalizedChatJid = bareAddress(chatJid) ?? chatJid.trim();
    _mamGlobalCompleteChatCalendarCandidates.remove(normalizedChatJid);
  ...
  bool _calendarSyncStateCoversInbound({
    required CalendarSyncInbound inbound,
    required bool isSelfCalendar,
    required String chatJid,
  }) {
    ...
    if (inboundId != null &&
        ...
        (state.lastHandledStanzaId == inboundId ||
            state.lastAppliedStanzaId == inboundId)) {
      return true;
    }
    final inboundTimestamp = inbound.receivedAt?.toUtc();
    final cursorTimestamp =
        state.lastHandledTimestamp ?? state.lastAppliedTimestamp;
    if (inboundTimestamp == null || cursorTimestamp == null) {
      return false;
    }
    final cursor = cursorTimestamp.toUtc();
    if (cursor.isAfter(inboundTimestamp)) {
      return true;
    }
    return cursor.isAtSameMomentAs(inboundTimestamp) &&
        (inboundId == null || inboundId.isEmpty);
  }
  ...
  _CalendarMamSession _calendarMamSessionFor({
    required String chatJid,
    required bool isSelfCalendar,
  }) {
    final normalizedJid = bareAddress(chatJid) ?? chatJid.trim();
    final scope = isSelfCalendar
        ? 'personal:$normalizedJid'
        : 'chat:$normalizedJid';
    return _calendarMamSessions.putIfAbsent(scope, _CalendarMamSession.new);
  }
  ...
  void _trackMamGlobalChatCalendarCoverageCandidate({
    ...
    if (!_mamGlobalSyncInFlight || isSelfCalendar) {
      return;
    }
    if (mamSession.inFlight) {
      return;
  ...
  String _calendarSyncChatJid(mox.MessageEvent event, String? accountJid) {
    final isGroupChat = event.type == 'groupchat';
    final to = bareAddress(event.to.toString()) ?? event.to.toBare().toString();
    final from =
        bareAddress(event.from.toString()) ?? event.from.toBare().toString();
    if (isGroupChat) {
      return from;
    }
    if (accountJid != null && accountJid.trim().isNotEmpty) {
      if (sameBareAddress(from, accountJid)) {
        return to;
      }
    }
    return from;
  ...
  Future<void> _ensureSharedPinArchiveBootstrapped({
    ...
    try {
      ...
      while (true) {
        ...
        final nextBefore = result.firstId;
        if (result.complete || nextBefore == null) {
          completed = true;
          break;
        }
  ...
  ///
  /// Returns the non-fatal calendar archive recovery outcome.
  Future<CalendarMamOutcome> rehydrateCalendarFromMam() async {
    final selfJid = myJid;
    if (selfJid == null) {
    ...
    final session = _calendarMamSessionFor(
      chatJid: selfJid,
      isSelfCalendar: true,
    );
    if (session.inFlight) {
      return CalendarMamOutcome.skippedInFlight;
    }
    ...
    try {
      ...
      final storedCalendarState = _readDirectPersonalCalendarStateOrNull();
      if (state.hasCompleteCoverage &&
          _mamGlobalSyncCompletedSinceConnect &&
          storedCalendarState != null &&
      ...
      if (recoveryTimestamp != null && state.canUseRecoveryCursor) {
        final completed = await _catchUpCalendarFromArchive(
          ...
          isMuc: false,
          after: _calendarMamResumeAfter(state, archiveJid: selfJid),
          mamSession: session,
          recordResumeId: (resumeId) =>
              _writePersonalCalendarArchiveResumeState(
                resumeId: resumeId,
                calendarJid: selfJid,
                archiveJid: selfJid,
              ),
      ...
      final completed = await _backfillCalendarFromArchive(
        jid: selfJid,
        isMuc: false,
        mamSession: session,
        recordResumeId: (resumeId) => _writePersonalCalendarArchiveResumeState(
          resumeId: resumeId,
          calendarJid: selfJid,
          archiveJid: selfJid,
        ),
      );
      if (!session.snapshotSeen && session.snapshotUnavailableNotified) {
        await _requestCalendarSnapshotFallback(selfJid);
      }
      final complete = _isPersonalBackfillComplete(
        completed: completed,
        snapshotSeen: session.snapshotSeen,
      );
      await _writePersonalCalendarBackfillCoverageState(
        complete: complete,
        snapshotSeen: session.snapshotSeen,
        calendarJid: selfJid,
        archiveJid: selfJid,
  ...
  Future<CalendarMamOutcome> rehydrateChatCalendarFromMam({
    required String chatJid,
    required ChatType chatType,
  }) async {
    final normalizedChatJid = bareAddress(chatJid) ?? chatJid.trim();
    ...
    try {
      ...
      final completed = recoveryTimestamp == null || !state.canUseRecoveryCursor
          ? await _backfillCalendarFromArchive(
              ...
              mamSession: session,
              recordResumeId: (resumeId) =>
                  _writeChatCalendarArchiveResumeState(
                    chatJid: normalizedChatJid,
                    resumeId: resumeId,
                  ),
            )
      ...
      _log.warning('Chat calendar rehydration failed.', error, stackTrace);
      await _writeChatCalendarCoverageState(
        chatJid: normalizedChatJid,
        complete: false,
      );
  ...
  Future<void> _writePersonalCalendarCoverageState({
  ...
  bool _isPersonalBackfillComplete({
    ...
    return _readPersonalCalendarSyncState().snapshotCoverageStatus ==
        CalendarSnapshotCoverageStatus.verified;
  }

  Future<void> _writePersonalCalendarBackfillCoverageState({
    required bool complete,
    required bool snapshotSeen,
    required String calendarJid,
    required String archiveJid,
  ...
  Future<bool> _catchUpCalendarFromArchive({
    ...
    required bool isMuc,
    required _CalendarMamSession mamSession,
    required Future<void> Function(String resumeId) recordResumeId,
    String? after,
  }) async {
    final trimmedAfter = after?.trim();
    String? afterId = trimmedAfter == null || trimmedAfter.isEmpty
        ? null
        : trimmedAfter;
    while (true) {
      mamSession.beginPage();
      final result = await fetchSinceFromArchive(
        jid: jid,
        since: since,
      ...
      if (mamSession.pageHadProcessingFailure) {
        return false;
      }
      final pageResumeId = result.lastId?.trim();
      if (pageResumeId != null && pageResumeId.isNotEmpty) {
        await recordResumeId(pageResumeId);
      }
      final nextAfter = result.lastId ?? afterId;
      if (result.complete) {
        return true;
      }
      if (nextAfter == null || nextAfter == afterId) {
        return false;
      }
      afterId = nextAfter;
    }
  ...
  String? _calendarMamResumeAfter(
    CalendarSyncState state, {
    required String archiveJid,
  }) {
    final stateArchiveJid = state.archiveJid?.trim();
    if (stateArchiveJid != null &&
        stateArchiveJid.isNotEmpty &&
        !sameBareAddress(stateArchiveJid, archiveJid)) {
      return null;
    }
    final candidate =
        state.lastArchiveResumeId ??
        state.lastHandledStanzaId ??
        state.lastAppliedStanzaId;
    final trimmed = candidate?.trim();
    return trimmed == null || trimmed.isEmpty ? null : trimmed;
  }

  Future<bool> _backfillCalendarFromArchive({
    required String jid,
    required bool isMuc,
    required _CalendarMamSession mamSession,
    required Future<void> Function(String resumeId) recordResumeId,
    ...
    while (true) {
      ...
      final newestResumeId = result.lastId?.trim();
      if (beforeId == null &&
          newestResumeId != null &&
          newestResumeId.isNotEmpty) {
        mamSession.newestArchiveResumeId = newestResumeId;
      }
      if (mamSession.snapshotSeen || result.complete) {
        final resumeId = mamSession.newestArchiveResumeId;
        if (resumeId != null) {
          await recordResumeId(resumeId);
        }
        return true;
      }
  ...
  Future<bool> _sendPinMutationToChat({
    ...
    final outboundMessageId = await _resolveOutboundPinMessageId(
      chatJid: chatJid,
  ...
  Future<String?> _normalizePinReferenceForChat({
    ...
    final normalizedChat = _normalizePinChatJid(chatJid);
    if (trimmed.isEmpty ||
        isLegacyWireMessageReferenceValue(trimmed) ||
        normalizedChat == null) {
      return null;
    }
    final identity = await _pinReferenceIdentityForMessageId(
      trimmed,
      chatJid: normalizedChat,
    );
    final canonicalId = identity.canonicalId.trim();
    if (canonicalId.isEmpty || isLegacyWireMessageReferenceValue(canonicalId)) {
      return null;
    }
  ...
  _pinReferenceIdentityForMessageId(String messageId, {String? chatJid}) async {
    final trimmed = messageId.trim();
    if (trimmed.isEmpty) {
      return (canonicalId: '', aliases: const <String>{});
    }
    final message = await _dbOpReturning<XmppDatabase, Message?>(
      (db) => db.getMessageByReferenceId(trimmed, chatJid: chatJid),
    );
    if (message == null) {
