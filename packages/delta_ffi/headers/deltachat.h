...
uint32_t axichat_dc_get_max_msg_id(dc_context_t* ctx);
char* axichat_dc_get_msg_ids_after(dc_context_t* ctx, uint32_t after_id, uint32_t limit);
...
void dc_imex(dc_context_t* ctx, int what, const char* param1, const char* param2);
void dc_stop_ongoing_process(dc_context_t* ctx);
...
char* dc_get_msg_html(dc_context_t* ctx, uint32_t msg_id);
...
int32_t dc_msg_get_showpadlock(const dc_msg_t* msg);
...
void dc_msg_skip_autocrypt(dc_msg_t* msg);
