...
void main() {
  testWidgets('showCalendarTaskShareSheet survives opener disposal', (
    tester,
  ) async {
    ...
    await tester.pumpWidget(
      _CalendarShareHarness(
        ...
        childBuilder: (context) => ElevatedButton(
          onPressed: () {
            unawaited(
              showCalendarTaskShareSheet(context: context, task: _task()),
            );
          },
          child: const Text('Open task share'),
        ),
      ),
    );

    await tester.tap(find.text('Open task share'));
