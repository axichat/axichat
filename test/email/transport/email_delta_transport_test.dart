...
void main() {
  ...
  test('refresh repair preserves unverifiable stored origins', () async {
    const chatId = 7;
    const msgId = 71;
    ...
    final stored = Message(
      ...
      body: '\u{1F4CE} image.png',
      originID: 'wrong@example.com',
      timestamp: DateTime.utc(2024, 1, 1),
      fileMetadataID: 'image-metadata',
    ...
    when(() => context.getChatlist()).thenAnswer((_) async => const []);
    when(
      () => context.getChatlist(flags: any(named: 'flags')),
    ).thenAnswer((_) async => const []);
    when(
      () => database.getEmailMessagesForOriginRepair(
        deltaAccountId: DeltaAccountDefaults.legacyId,
        limit: 500,
      ),
    ...
    await transport.refreshChatlistSnapshot();
  ...
  test('refresh repair merges only exact safe origin duplicates', () async {
    const chatId = 8;
    const msgId = 80;
    final events = stubInitializedSingleContext();
    final stored = Message(
    ...
    final corrupted = Message(
      stanzaID: 'wrong-origin-owner',
      senderJid: 'mallory@example.com',
    ...
    ).thenAnswer((_) async => [corrupted, duplicate]);
    when(() => database.updateMessage(any())).thenAnswer((_) async {});
    when(
    ...
    when(
      () => database.repairChatSummaryPreservingTimestamp(stored.chatJid),
    ).thenAnswer((_) async {});
    ...
    final updated =
        verify(() => database.updateMessage(captureAny())).captured.single
            as Message;
    expect(updated.stanzaID, stored.stanzaID);
