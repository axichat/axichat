...
void main() {
  ...
  group('direct server stanza ids', () {
    test(
      'saveMessage backfills direct server stanza ids and reference lookup resolves them',
      () async {
        await connectSuccessfully(xmppService);

        const chatJid = 'peer@axi.im';
        const localStanzaId = 'local-stanza-id';
        const serverStanzaId = 'server-stanza-id';
        ...
        await database.saveMessage(
          message.copyWith(serverStanzaId: serverStanzaId),
        );

        final stored = await database.getMessageByStanzaID(localStanzaId);
        final byReference = await database.getMessageByReferenceId(
          serverStanzaId,
          chatJid: chatJid,
        );

        expect(stored?.serverStanzaId, serverStanzaId);
        expect(byReference?.stanzaID, localStanzaId);
      },
    );

    test(
      'markIncomingMessagesDisplayedThroughServerStanzaId updates only inbound unread messages',
      () async {
        await connectSuccessfully(xmppService);
        ...
        final chat = Chat(
          ...
          title: 'Peer',
          type: ChatType.chat,
          lastChangeTimestamp: DateTime.utc(2026, 3, 19, 12),
          contactJid: chatJid,
        );
        await database.createChat(chat);

        final incomingOne = Message(
          stanzaID: 'local-1',
          senderJid: chatJid,
          chatJid: chatJid,
          timestamp: DateTime.utc(2026, 3, 19, 12, 0, 0),
          body: 'one',
          serverStanzaId: 'server-1',
        );
        final incomingTwo = Message(
          stanzaID: 'local-2',
          senderJid: chatJid,
          chatJid: chatJid,
          timestamp: DateTime.utc(2026, 3, 19, 12, 1, 0),
          body: 'two',
          serverStanzaId: 'server-2',
        );
        final outgoing = Message(
          stanzaID: 'local-3',
          senderJid: selfJid,
          chatJid: chatJid,
          timestamp: DateTime.utc(2026, 3, 19, 12, 2, 0),
          body: 'three',
          serverStanzaId: 'server-3',
        );

        await database.saveMessage(incomingOne, selfJid: selfJid);
        await database.saveMessage(incomingTwo, selfJid: selfJid);
        await database.saveMessage(outgoing, selfJid: selfJid);

        await database.markIncomingMessagesDisplayedThroughServerStanzaId(
          serverStanzaId: 'server-3',
          chatJid: chatJid,
          selfJid: selfJid,
        );

        final refreshedIncomingOne = await database.getMessageByStanzaID(
          incomingOne.stanzaID,
        );
        final refreshedIncomingTwo = await database.getMessageByStanzaID(
          incomingTwo.stanzaID,
        );
        final refreshedOutgoing = await database.getMessageByStanzaID(
          outgoing.stanzaID,
        );
        final refreshedChat = await database.getChat(chatJid);

        expect(refreshedIncomingOne?.displayed, isTrue);
        expect(refreshedIncomingTwo?.displayed, isTrue);
        expect(refreshedOutgoing?.displayed, isFalse);
        expect(refreshedChat?.unreadCount, 0);
      },
    );

    test(
      'server stanza ordering compares same-timestamp messages by insertion order',
      () async {
        await connectSuccessfully(xmppService);

        const chatJid = 'peer@axi.im';
        final first = Message(
          stanzaID: 'local-first',
          senderJid: chatJid,
          chatJid: chatJid,
          timestamp: DateTime.utc(2026, 3, 19, 12),
          body: 'first',
          serverStanzaId: 'server-first',
        );
        final second = Message(
          stanzaID: 'local-second',
          senderJid: chatJid,
          chatJid: chatJid,
          timestamp: DateTime.utc(2026, 3, 19, 12),
          body: 'second',
          serverStanzaId: 'server-second',
        );

        await database.saveMessage(first);
        await database.saveMessage(second);

        final secondCoversFirst = await database.isServerStanzaAtOrAfterMessage(
          serverStanzaId: 'server-second',
          stanzaID: 'local-first',
          chatJid: chatJid,
        );
        final firstCoversSecond = await database.isServerStanzaAtOrAfterMessage(
          serverStanzaId: 'server-first',
          stanzaID: 'local-second',
          chatJid: chatJid,
        );

        expect(secondCoversFirst, isTrue);
        expect(firstCoversSecond, isFalse);
      },
    );
  });
