...
void main() {
  ...
  group('Direct invite serialization (XEP-0249)', () {
    test('DINV-010 [HP] direct invite XML includes required attributes', () {
      ...
      expect(node.attributes[_directInviteRoomAttr], equals(_roomJid));
      expect(
    ...
    test('DINV-011 [UP] missing jid rejects the direct invite', () {
      final stanza = mox.Stanza.message(
        children: [
          mox.XMLNode.xmlns(tag: _directInviteTag, xmlns: _directInviteXmlns),
        ],
      );
    ...
    test('DINV-012 [HP] optional reason/password are trimmed', () {
      const data = DirectMucInviteData(
        roomJid: _roomJid,
        reason: _inviteReasonRaw,
        password: _invitePasswordRaw,
    ...
    test('DINV-015 [EC] continue=false omits the continue attribute', () {
      const data = DirectMucInviteData(roomJid: _roomJid, continueFlag: false);
      final node = data.toXml();

      expect(node.attributes.containsKey(_directInviteContinueAttr), isFalse);
    });

    test('DINV-016 [UP] invalid continue values are ignored', () {
      final stanza = mox.Stanza.message(
        children: [
          mox.XMLNode.xmlns(
  ...
  group('Axi invite serialization', () {
    test('DINV-013 [HP] Axi invites trim optional fields', () {
  ...
  group('Invite parsing into message models', () {
    ...
    test('DINV-020 [HP] Axi invite payload yields MUC invite metadata', () {
      final event = _createMessageEvent(
        from: mox.JID.fromString(_inviterJid),
        to: mox.JID.fromString(_accountBareJid),
        extensions: [
      ...
      expect(message.pseudoMessageType, equals(PseudoMessageType.mucInvite));
      expect(message.pseudoMessageData?['roomJid'], equals(_roomJid));
      expect(message.pseudoMessageData?['token'], equals(_inviteTokenTrimmed));
    ...
    test(
      ...
      () {
        final event = _createMessageEvent(
          ...
          to: mox.JID.fromString(_accountBareJid),
          extensions: const [mox.OccupantIdData(_occupantId)],
